...
def draw_polygons( polygons, screen, zbuffer, view, ambient, symbols, reflect):
    ...
    while point < len(polygons) - 2:
        ...
        for symbol in symbols:
            if symbols[symbol] == 'camera':
                vnorm = calculate_view(polygons, point, symbols)[:]
                nc = False
        if nc: vnorm = [0, 0, 1]
        #print(cnorm)
        #print normal
        if normal[2] > 0:
            color = get_lighting(normal, vnorm, ambient, symbols, reflect )
...
def add_mesh(polygons, f):
    ...
    for l in lines:
    	if l[0] == 'v':
    	    e = l.split()
    	    v.append((float(e[1]), float(e[2]), float(e[3])))
    for l in lines:
    	if l[0] == 'f':
    	    e = l.split()
    	    p = e[1:]
    	    ...
    	    for i in range(1,len(p) - 1):
    	    	add_polygon(polygons, v[int(p[0]) - 1][0], v[int(p[0]) - 1][1], v[int(p[0]) - 1][2], v[int(p[i])- 1][0], v[int(p[i])- 1][1], v[int(p[i]) - 1][2], v[int(p[i + 1]) - 1][0], v[int(p[i + 1]) - 1][1], v[int(p[i + 1]) - 1][2])
