...
def get_lighting(normal, view, ambient, symbols, reflect ):
    ...
    for symbol in symbols:
    	if symbols[symbol][0] == 'light':
    	    lights.append(symbols[symbol][1])
    n = normal[:]
    normalize(n)
    normalize(view)
    r = symbols[reflect][1]
...
def calculate_view(polygons, i, symbols):

    V = [0, 0, 0]
    O = [0, 0, 0]
    O[0] = float(polygons[i][0] + polygons[i + 1][0] + polygons[i + 2][0]) / 3
    O[1] = float(polygons[i][1] + polygons[i + 1][1] + polygons[i + 2][1]) / 3
    O[2] = float(polygons[i][2] + polygons[i + 1][2] + polygons[i + 2][2]) / 3
    V[0] = abs(symbols['camera'][1]['position'][0] - O[0])
    V[1] = abs(symbols['camera'][1]['position'][1] - O[1])
    V[2] = abs(symbols['camera'][1]['position'][2] - O[2])
    normalize(V)
    return V
