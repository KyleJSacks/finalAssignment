...
def p_command_cmove(p):
    """command : CMOVE NUMBER NUMBER NUMBER SYMBOL
               | CMOVE NUMBER NUMBER NUMBER"""
    cmd = {'op' : p[1], 'args' : p[2:5], 'knob' : None}
    if len(p) == 6:
        cmd['knob'] = p[5]
        symbols[p[5]] = ['knob', 0]
...
def p_command_crotate(p):
    """command : CROTATE XYZ NUMBER SYMBOL
                 | CROTATE XYZ NUMBER"""
    cmd = {'op' : p[1], 'args' : p[2:4], 'knob' : None}
    if len(p) == 5:
        cmd['knob'] = p[4]
...
def p_command_camera(p):
    ...
    symbols['camera'] = ['camera', {'position': p[2:5], 'aim': p[5:]} ]
    commands.append({'op':p[1], 'args':None})
