...
def new_zbuffer( cz = float('-inf'), width = XRES, height = YRES ):
    zb = []
    for y in range( height ):
        row = [ cz for x in range(width) ]
        zb.append( row )
    return zb
...
def plot( screen, zbuffer, color, x, y, z ):
    ...
    if ( x >= 0 and x < XRES and newy >= 0 and newy < YRES and zbuffer[newy][x] <= z and zbuffer[newy][x]):
        screen[newy][x] = color[:]
        zbuffer[newy][x] = z
